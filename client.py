import socket
import rsa
import json
import os

# 服务器配置
HOST = '127.0.0.1'
PORT = 65432

def generate_keys():
    """生成 RSA 公私钥对 (512位用于实验展示)"""
    print("[系统] 正在生成 RSA 密钥对...")
    (pub_key, priv_key) = rsa.newkeys(512)
    return pub_key, priv_key

def register(user_id, pub_key):
    """向服务器注册公钥"""
    print(f"\n[注册] 正在为用户 {user_id} 发起公钥注册...")
    try:
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
            s.connect((HOST, PORT))
            pub_key_pem = pub_key.save_pkcs1().decode('utf-8')
            payload = {
                "type": "register",
                "user_id": user_id,
                "public_key": pub_key_pem
            }
            s.sendall(json.dumps(payload).encode('utf-8'))
            response = json.loads(s.recv(4096).decode('utf-8'))
            print(f"[注册] 服务器响应: {response.get('message')}")
            return response.get('status') == 'success'
    except Exception as e:
        print(f"[错误] 注册过程中发生异常: {e}")
        return False

def authenticate(user_id, priv_key, label="认证"):
    """执行挑战-应答身份认证流程"""
    print(f"\n[{label}] 用户 {user_id} 发起接入认证请求...")
    try:
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
            s.connect((HOST, PORT))
            
            # 1. 发送初步接入请求
            auth_request = {
                "type": "authenticate",
                "user_id": user_id
            }
            s.sendall(json.dumps(auth_request).encode('utf-8'))
            
            # 2. 接收挑战码或拒绝信息
            resp_data = s.recv(4096).decode('utf-8')
            if not resp_data:
                return False
            response = json.loads(resp_data)
            
            if response.get('status') == 'fail':
                print(f"[{label}] 认证被服务器拒绝: {response.get('message')}")
                return False

            challenge_hex = response.get('challenge')
            print(f"[{label}] 收到服务器随机数挑战: {challenge_hex}")
            challenge_bytes = bytes.fromhex(challenge_hex)

            # 3. 使用私钥对挑战码进行数字签名 (SHA-1)
            print(f"[{label}] 正在使用私钥生成数字签名...")
            signature = rsa.sign(challenge_bytes, priv_key, 'SHA-1')
            
            # 4. 发送签名结果
            sig_payload = {
                "signature": signature.hex()
            }
            s.sendall(json.dumps(sig_payload).encode('utf-8'))
            
            # 5. 接收最终结论
            final_resp_data = s.recv(4096).decode('utf-8')
            if not final_resp_data:
                return False
            final_resp = json.loads(final_resp_data)
            print(f"[{label}] 认证最终结论: {final_resp.get('message')}")
            return final_resp.get('status') == 'success'

    except Exception as e:
        print(f"[{label}] 流程异常: {e}")
        return False

if __name__ == "__main__":
    print("=== 无线网络跨层联动身份认证实验 - 客户端模拟程序 ===")

    # 准备密钥对
    pub_a, priv_a = generate_keys() # 用户 A (合法用户)
    pub_b, priv_b = generate_keys() # 用户 B (未注册用户)
    pub_c, priv_c = generate_keys() # 用户 C (伪装攻击者)

    # --- 场景 1: 用户A (合法用户) ---
    # 完成公钥注册，使用正确的私钥进行签名认证
    print("\n--- 场景 1: 用户A (合法用户) 正常流程 ---")
    if register("User_A", pub_a):
        authenticate("User_A", priv_a, label="场景1")

    # --- 场景 2: 用户B (未注册用户) ---
    # 未在服务器中注册公钥信息，尝试直接接入系统
    print("\n--- 场景 2: 用户B (未注册用户) 直接接入 ---")
    authenticate("User_B", priv_b, label="场景2")

    # --- 场景 3: 用户C (伪装攻击者) ---
    # 持有自己的私钥，试图伪造用户A的ID发起认证
    print("\n--- 场景 3: 用户C (攻击者) 冒充 User_A ---")
    # 即使攻击者注册了自己的公钥 (User_C)，冒充 A 时也会因为私钥不匹配失败
    register("User_C", pub_c) 
    authenticate("User_A", priv_c, label="场景3")

    print("\n=== 所有测试场景模拟结束 ===")
